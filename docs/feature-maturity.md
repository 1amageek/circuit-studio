...
| Headless round-trip harness | 4 | `HeadlessRoundTripService` runs schematic net extraction, SPICE generation, pre-layout simulation, auto layout, external signoff command execution, persisted signoff approval, strict pre-PEX verification, PEX IR injection, post-layout simulation, and writes a machine-readable manifest with artifacts and gate status; failed gates now persist manifests with failed/skipped stage state before throwing; `circuit-studio-flow-runner` exposes the voltage-divider and CMOS-inverter pass paths as a standalone command-line entrypoint | `HeadlessRoundTripServiceTests` covers CMOS inverter transient, voltage divider OP, pre-PEX failure, empty PEX IR failure, and post-layout simulation failure; `swift run circuit-studio-flow-runner --fixture voltage-divider` and `swift run circuit-studio-flow-runner --fixture cmos-inverter` both pass | Real PEX backend and real signoff decks are still mock-backed; failure manifests are covered for headless harness gates but not real-tool corpus failures | Add real PEX artifacts, real signoff deck smoke tests, and broader circuit fixtures |
| Full LVS / DRC preflight | 4 | `PhysicalVerificationService`, DRC violation summaries, schematic-to-layout DesignUnit checks, stale-layout detection, actual layout instance validation, physically realized net validation, hierarchy topology error reporting, hierarchical connectivity extraction through shapes/vias/cut-shapes/pins/labels/instance pins, physical short/open detection, terminal mismatch reporting, explicit Full LVS skip diagnostics, missing/misnamed external port detection, invalid imported terminal reporting, duplicate imported terminal reporting, metadata-backed polygon terminal recognition, raw MOS `ACTIVE ∩ POLY` extraction, source/drain/gate/bulk terminal inference for current sample-process fixtures, numeric GDS layer alias normalization, shared-diffusion two-device extraction, raw resistor extraction, MOS W/L/kind/`nf` comparison, resistor value comparison, external signoff command execution, log artifact capture, existing signoff log import, report normalization, approval persistence, and explicit human approval gating before PEX | `PhysicalVerificationServiceTests`, `ExternalSignoffCommandServiceTests`, `ExternalSignoffArtifactServiceTests`, `CMOSInverterFlowTests`; lower-level `LayoutIOTests` pass | Real foundry PDK extraction rules, golden GDS/OASIS regression corpora, real signoff deck fixtures, and UI review workflows are not implemented | Add foundry-style extraction deck fixtures, golden GDS/OASIS corpus tests, real signoff deck smoke tests, and review UI approval tests |
...
The strongest `circuit-studio` areas are model/catalog, netlist generation, process configuration, and representative simulation flows. The DRC/LVS/PEX/post-layout path now has a strict headless round-trip harness that carries both a CMOS inverter and a voltage divider through artifact-producing stages without forced continuation, failed gates leave manifests for agent/human review, externally produced signoff logs can be imported into the same approval gate, and the pass-path corpus can be run from the standalone `circuit-studio-flow-runner` CLI. Real foundry decks, real PEX artifacts, broader circuit fixtures, and multi-corner post-layout review are still future work.
...
| 2 | Add real PEX artifact fixtures and pre/post waveform comparison tests across PEX corners |
