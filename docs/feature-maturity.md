...
| CircuitStudio core tests | `perl -e 'alarm 120; exec @ARGV' swift test --filter CircuitStudioCoreTests` | 147 pass, 4 skipped |
...
| Headless round-trip harness | 4 | `HeadlessRoundTripService` runs schematic net extraction, SPICE generation, pre-layout simulation, auto layout, external signoff command execution or imported signoff review loading, persisted signoff approval, strict pre-PEX verification, PEX IR injection, post-layout simulation, pre/post waveform comparison, optional absolute/relative comparison limits, and writes a machine-readable manifest with artifacts, captured input artifact paths, original source paths, comparison report path, and gate status; failed gates now persist manifests with failed/skipped stage state before throwing; `circuit-studio-flow-runner` exposes the voltage-divider and CMOS-inverter pass paths as a standalone command-line entrypoint and can load saved signoff logs plus a saved PEX manifest/IR with explicit `--approve-signoff` and `--max-abs-delta` / `--max-rel-delta` comparison gates | `HeadlessRoundTripServiceTests` covers CMOS inverter transient, voltage divider OP, imported signoff review, captured artifact paths, comparison artifact, comparison write failure, comparison-limit failure, pre-PEX failure, empty PEX IR failure, and post-layout simulation failure; `PostLayoutComparisonServiceTests` covers sweep mismatch rejection and limit diagnostics; CLI approved pass, unapproved failure, relaxed comparison-limit pass, and strict comparison-limit failure are verified | Real PEX backend and real signoff decks are still mock-backed; failure manifests are covered for headless harness gates but not real-tool corpus failures; comparison reports `not-comparable` instead of interpolating when sweep grids differ | Add real PEX backend smoke tests, real signoff deck smoke tests, broader circuit fixtures, interpolation, and multi-corner comparison gates |
...
| Post-layout simulation | 4 | `PostLayoutSimulationService` merges base SPICE and unit-normalized PEX parasitic IR, renders extracted R/C/Ccoupling elements, runs CoreSpice analysis, and `PostLayoutComparisonService` records pre/post common-variable deltas only for matching sweep grids; optional max absolute/relative delta limits can fail the post-layout comparison stage | `PostLayoutSimulationServiceTests`, `PostLayoutComparisonServiceTests`, `HeadlessRoundTripServiceTests`, `CMOSInverterFlowTests`; CLI relaxed/strict comparison-limit runs | Model/subckt-aware parasitic insertion, corner sweeps, interpolation, and large PEX deck performance are not covered | Add interpolation and multi-corner post-layout comparison tests |
...
The strongest `circuit-studio` areas are model/catalog, netlist generation, process configuration, and representative simulation flows. The DRC/LVS/PEX/post-layout path now has a strict headless round-trip harness that carries both a CMOS inverter and a voltage divider through artifact-producing stages without forced continuation, failed gates leave manifests for agent/human review, externally produced signoff logs can be imported into the same approval gate only with explicit approval, the pass-path corpus can be run from the standalone `circuit-studio-flow-runner` CLI, and the voltage-divider CLI path can consume saved golden signoff logs plus a saved golden PEX manifest/IR artifact while capturing those input artifacts and pre/post waveform deltas in the manifest. Post-layout comparison can now be informational or a hard absolute/relative delta gate. Real foundry decks, real extractor backends, broader circuit fixtures, interpolation, and multi-corner post-layout review are still future work.
...
| 2 | Add real PEX backend smoke tests and multi-corner pre/post waveform gates |
