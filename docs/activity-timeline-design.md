...
| `XcircuiteSimulationRunRecorder` | Persists simulation requests and input netlists before execution, then atomically finalizes immutable summaries, waveforms, errors, measured producer identity, effective-environment fingerprint, and run status. Terminal runs reject every later artifact write. | Simulation Activity can be reconstructed from existing run evidence. Do not add a second simulation log. |
