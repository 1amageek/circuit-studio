...
Timing artifacts live below the active run directory. Existing headless round trips use `.xcircuite/flow-runs/<run-id>/`; Agent-driven library runs may use `.xcircuite/runs/<run-id>/`. The format below is relative to either run directory root.
