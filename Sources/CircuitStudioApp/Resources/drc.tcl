...
# Fail loudly if the cell did not actually load. A missing cell (absent from the
# PDK search path or not present in the GDS) leaves Magic with an empty 1x1
# placeholder; reporting that as a clean DRC would be a silent false pass.
lassign [box values] llx lly urx ury
if {[expr {($urx - $llx) * ($ury - $lly)}] <= 1} {
    puts "ERROR rule=DRIVER message=\"cell not found or empty: $cell\""
    quit -noprompt
}
